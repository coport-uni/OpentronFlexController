...
# Run from the project root so ruff resolves this project's ruff.toml.
# Ruff's config discovery starts at the working directory, so a stray
# config in whatever directory the hook inherits would otherwise win.
